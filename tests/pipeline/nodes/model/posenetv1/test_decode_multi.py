...
class TestDecodeMulti:
    ...
    def test_within_nms_radius_fast(self, root_image_coords):
        ...
        check = _within_nms_radius_fast(pose_coords, squared_nms_radius,
                                        root_image_coords)
    ...
    def test_get_instance_score_fast(self, dst_scores, dst_keypoints):
        ...
        pose_score = _get_instance_score_fast(exist_pose_coords,
                                              squared_nms_radius,
                                              keypoint_scores,
                                              keypoint_coords)
    ...
    def test_change_dimensions(self):
        ...
        new_offsets, new_dfwd, new_dbwd = _change_dimensions(scores,
                                                             offsets,
                                                             displacements_fwd,
                                                             displacements_bwd)
    ...
    def test_sort_scored_parts(self):
        ...
        expected_parts = [(0.924, 12, np.array([5, 11])),
                          (0.806, 0, np.array([15, 12])),
                          (0.490, 1, np.array([3, 15])),
                          (0.299, 2, np.array([4, 3])),
                          (0.058, 15, np.array([10,  0]))]
        scored_parts = _sort_scored_parts(sample_parts)
        npt.assert_array_equal(np.array(scored_parts, dtype='object')[:, :2],
