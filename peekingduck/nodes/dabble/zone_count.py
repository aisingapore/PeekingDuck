...
class Node(AbstractNode):
    ...
    def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        points = [Point(point) for point in inputs["btm_midpoint"]]
        ...
        return {"zones": self.zone_polygon_points, "zone_count": zone_counts}
    ...
    def _create_zone(self, zone: List[List[Union[float, int]]]) -> Zone:
        ...
        if all(all(0 <= i <= 1 for i in coords) for coords in zone):
            ...
            zone_points = list(map(self._get_pixel_coords, zone))
        elif all(
            all((isinstance(i, int) and i >= 0) for i in coords) for coords in zone
        ):
            # list is in pixel value.
            zone_points = list(map(tuple, zone))  # type: ignore
        else:
            raise ValueError(
    ...
    def _get_pixel_coords(self, coords: List[Union[float, int]]) -> Tuple[int, int]:
        """Returns the pixel position of the zone points."""
        return int(coords[0] * self.resolution[0]), int(coords[1] * self.resolution[1])
