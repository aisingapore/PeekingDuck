...
class YoloModel:
    ...
    def __init__(self, config):
        ...
        if not checker.has_weights(config['root'],
                                   config['weights_dir']):
            logger.info('---no yolo weights detected. proceeding to download...---')
            downloader.download_weights(config['root'],
                                        config['weights_id'])
            logger.info('---yolo weights download complete.---')
        ...
        logger.info('yolo model detecting ids: {}'.format(self.detect_ids))
