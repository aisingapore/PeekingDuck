...
def rescale_image(
    ...
    scale = np.array(
        [source_img.shape[1] / target_width, source_img.shape[0] / target_height]
    )

    image_processed = _rescale_image(
        source_img, target_width, target_height, model_type
    )

    return image_processed, scale


def _get_valid_resolution(
    width: float, height: float, output_stride: int = 16
) -> Tuple[int, int]:
...
def _rescale_image(
    ...
    image_processed = image_processed.reshape(1, target_height, target_width, 3)
