...
def draw_text(img: np.ndarray, text: str, pos: tuple, posType: int) -> None:
    ...
    text_size = cv2.getTextSize(text, fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=font_scale, thickness = line_thickness)
    ...
    if posType == TOP_LEFT:
        ...
        cv2.rectangle(
            box_img,
            (pos[0], pos[1]),
            (pos[0] + text_width + 5 + 5, pos[1] + text_height + 5 + 5),
            (0, 0, 0),
            cv2.FILLED,
        )
        # apply the overlay
