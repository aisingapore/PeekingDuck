...
class Node(AbstractNode):
    ...
    def _get_target_attr(self, inputs: Dict[str, Any]) -> Union[int, float, list, dict]:
        """Gets the target attributes from inputs."""
        if self.target["get"]:
...
def _apply_func(
    target_attr: Union[int, float, list, dict], apply: Dict[str, Any]
    ...
    # if function == "conditional_count"
    _check_type(target_attr, function, list)
    return _conditional_count(target_attr, condition)


def _check_type(
    target_attr: Union[int, float, list, dict], function: str, *types: type
) -> None:
...
def _conditional_count(
    target_attr: Union[int, float, List[Any], Dict[str, Any]], condition: Dict[str, str]
) -> int:
    """Counts the number of elements in a list given an operator and operand for comparison."""
    count = 0
    op_func = OPS[condition["operator"]]
    for item in target_attr:
        if op_func(item, condition["operand"]):
            count += 1
    return count
