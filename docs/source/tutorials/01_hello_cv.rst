...
PeekingDuck has a Viewer which can be activated using the CLI ``--viewer`` option:
...
A screenshot of the Viewer and its GUI components is shown below:
   ...
   .. figure:: /assets/tutorials/viewer.png
      :alt: PeekingDuck Viewer Screenshot

      PeekingDuck Viewer Screenshot

When PeekingDuck is executing the current pipeline, a progress bar will appear below the 
output video.

If the length of the pipeline input is deterministic, like using a video file as the source, 
then the progress bar will function like a normal progress bar moving from start to end.
Upon completion, the progress bar will be replaced by a slider.

If the length is non-deterministic, like capturing a webcam video, then the progress bar
...
The user can examine the output video of the executed pipeline by using the
``Play/Stop`` button to replay the video.
The user may also scrub through the video using the slider to go directly to the frames 
of interest.

The ``Zoom In/Zoom Out`` buttons allow the user to adjust the size of the input.
Alternatively, the user can also manually resize the Viewer window to accommodate the input.

To end the Viewer, close the Viewer window.
